...
public abstract class AsyncHttpResponseHandler implements ResponseHandlerInterface {
    ...
    public void setUseSynchronousMode(boolean sync) {
        if (!sync && this.looper == null) {
            sync = true;
            AsyncHttpClient.log.m1890w(LOG_TAG, "Current thread has not called Looper.prepare(). Forcing synchronous mode.");
        }
    ...
    public void onProgress(long bytesWritten, long totalSize) {
        ...
        objArr[SUCCESS_MESSAGE] = Long.valueOf(bytesWritten);
        objArr[FAILURE_MESSAGE] = Long.valueOf(totalSize);
        objArr[START_MESSAGE] = Double.valueOf(totalSize > 0 ? ((((double) bytesWritten) * 1.0d) / ((double) totalSize)) * 100.0d : -1.0d);
        logInterface.m1888v(str, String.format(str2, objArr));
    }
    ...
    public void onRetry(int retryNo) {
        ...
        objArr[SUCCESS_MESSAGE] = Integer.valueOf(retryNo);
        logInterface.m1882d(str, String.format("Request retry no. %d", objArr));
    }

    public void onCancel() {
        AsyncHttpClient.log.m1882d(LOG_TAG, "Request got cancelled");
    }

    public void onUserException(Throwable error) {
        AsyncHttpClient.log.m1885e(LOG_TAG, "User-space exception detected!", error);
        throw new RuntimeException(error);
    }

    public final void sendProgressMessage(long bytesWritten, long bytesTotal) {
        Object obj = new Object[START_MESSAGE];
        obj[SUCCESS_MESSAGE] = Long.valueOf(bytesWritten);
        obj[FAILURE_MESSAGE] = Long.valueOf(bytesTotal);
    ...
    public final void sendSuccessMessage(int statusCode, Header[] headers, byte[] responseBytes) {
        ...
        obj[SUCCESS_MESSAGE] = Integer.valueOf(statusCode);
        obj[FAILURE_MESSAGE] = headers;
        obj[START_MESSAGE] = responseBytes;
    ...
    public final void sendRetryMessage(int retryNo) {
        ...
        obj[SUCCESS_MESSAGE] = Integer.valueOf(retryNo);
            ...
            switch (message.what) {
                case SUCCESS_MESSAGE /*0*/:
                    ...
                    if (response == null || response.length < FINISH_MESSAGE) {
                        AsyncHttpClient.log.m1884e(LOG_TAG, "SUCCESS_MESSAGE didn't got enough params");
                        return;
                    } else {
                        onSuccess(((Integer) response[SUCCESS_MESSAGE]).intValue(), (Header[]) response[FAILURE_MESSAGE], (byte[]) response[START_MESSAGE]);
                        return;
                    }
                case FAILURE_MESSAGE /*1*/:
                    response = (Object[]) message.obj;
                    if (response == null || response.length < PROGRESS_MESSAGE) {
                        AsyncHttpClient.log.m1884e(LOG_TAG, "FAILURE_MESSAGE didn't got enough params");
                        return;
                    } else {
                        onFailure(((Integer) response[SUCCESS_MESSAGE]).intValue(), (Header[]) response[FAILURE_MESSAGE], (byte[]) response[START_MESSAGE], (Throwable) response[FINISH_MESSAGE]);
                        return;
                    }
                ...
                case PROGRESS_MESSAGE /*4*/:
                    ...
                    if (response == null || response.length < START_MESSAGE) {
                        AsyncHttpClient.log.m1884e(LOG_TAG, "PROGRESS_MESSAGE didn't got enough params");
                        return;
                    } else {
                        onProgress(((Long) response[SUCCESS_MESSAGE]).longValue(), ((Long) response[FAILURE_MESSAGE]).longValue());
                        return;
                    }
                case RETRY_MESSAGE /*5*/:
                    response = (Object[]) message.obj;
                    if (response == null || response.length != FAILURE_MESSAGE) {
                        AsyncHttpClient.log.m1884e(LOG_TAG, "RETRY_MESSAGE didn't get enough params");
                        return;
                    } else {
                        onRetry(((Integer) response[SUCCESS_MESSAGE]).intValue());
                        return;
                    }
    ...
    byte[] getResponseData(HttpEntity entity) throws IOException {
        ...
        if (entity != null) {
            ...
            if (instream != null) {
                ...
                try {
                    ByteArrayBuffer buffer = new ByteArrayBuffer(contentLength <= 0 ? BUFFER_SIZE : (int) contentLength);
                    ...
                    while (true) {
                        ...
                        long j;
                        count += (long) l;
                        buffer.append(tmp, SUCCESS_MESSAGE, l);
                        if (contentLength <= 0) {
                            j = 1;
                        } else {
                            j = contentLength;
