...
public final class Base64 {
    ...
    public static String encode(byte[] binaryData) {
        ...
        while (i < numberTriplets) {
            ...
            if ((b1 & SIGN) == 0) {
                ...
                val1 = (byte) ((b1 >> 2) ^ Downloads.STATUS_RUNNING);
            }
            if ((b2 & SIGN) == 0) {
    ...
    public static byte[] decode(String encoded) {
        if (encoded == null) {
            Log.e("11111111111111111111111111111111", Constants.VIA_TO_TYPE_QQ_GROUP);
            return null;
        }
        char[] base64Data = encoded.toCharArray();
        int len = removeWhiteSpace(base64Data);
        if (len % FOURBYTE != 0) {
            Log.e("11111111111111111111111111111111", Constants.VIA_SSO_LOGIN);
            return null;
        }
        ...
        while (i < numberQuadruple - 1) {
            ...
            Log.e("11111111111111111111111111111111", Constants.VIA_TO_TYPE_QQ_DISCUSS_GROUP);
            return null;
        }
        dataIndex = dataIndex2 + 1;
        d1 = base64Data[dataIndex2];
        if (isData(d1)) {
            ...
            if (isData(d2)) {
                ...
                if (isData(d3) && isData(d4)) {
                    ...
                    if ((b2 & 15) != 0) {
                        Log.e("11111111111111111111111111111111", Constants.VIA_SHARE_TYPE_TEXT);
                        return null;
                    }
                    tmp = new byte[((i * 3) + 1)];
                    ...
                    Log.e("11111111111111111111111111111111", "7");
                    return null;
                } else {
                    b3 = base64Alphabet[d3];
                    if ((b3 & 3) != 0) {
                        Log.e("11111111111111111111111111111111", Constants.VIA_SHARE_TYPE_INFO);
                        return null;
                    }
                    tmp = new byte[((i * 3) + 2)];
                    System.arraycopy(decodedData, 0, tmp, 0, i * 3);
