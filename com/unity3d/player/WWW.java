...
class WWW extends Thread {
    ...
    protected boolean headerCallback(String str, String str2) {
        ...
        stringBuilder.append("\n\r");
        return headerCallback(this.f2230b, stringBuilder.toString());
    }
    ...
    protected void progressCallback(int i, int i2, int i3, int i4, long j, long j2) {
        ...
        progressCallback(this.f2230b, f2, f, d, i4);
    }

    protected boolean readCallback(byte[] bArr, int i) {
        return readCallback(this.f2230b, bArr, i);
    }
    ...
    public void run() {
        try {
            ...
            errorCallback(this.f2230b, "Error: " + th.toString());
        }
    }
    ...
    public void runSafe() {
        ...
        this.f2229a = i;
        if (i > 5) {
            errorCallback(this.f2230b, "Too many redirects");
            return;
        }
        try {
            URL url = new URL(this.f2231c);
            URLConnection openConnection = url.openConnection();
            if (!url.getProtocol().equalsIgnoreCase("file") || url.getHost() == null || url.getHost().length() == 0) {
                int min;
                HttpURLConnection httpURLConnection;
                if (this.f2233e != null) {
                    for (Entry entry : this.f2233e.entrySet()) {
                ...
                if (this.f2232d != null) {
                    ...
                    try {
                        ...
                        int i2 = 0;
                        while (i2 < this.f2232d.length) {
                            min = Math.min(1428, this.f2232d.length - i2);
                            outputStream.write(this.f2232d, i2, min);
                            i2 += min;
                            progressCallback(i2, this.f2232d.length, 0, 0, 0, 0);
                        }
                    } catch (Exception e) {
                        errorCallback(this.f2230b, e.toString());
                        return;
                    }
                }
                    ...
                    try {
                        ...
                        if (headerFields != null && (min == 301 || min == 302)) {
                            ...
                            if (!(list == null || list.isEmpty())) {
                                ...
                                this.f2231c = (String) list.get(0);
                                run();
                                return;
                            }
                        }
                    } catch (IOException e2) {
                        errorCallback(this.f2230b, e2.toString());
                        return;
                    }
                }
                ...
                if ((headerFields2 == null || !headerFields2.containsKey(C0182d.f422d)) && openConnection.getContentType() != null) {
                    headerCallback = headerCallback || headerCallback(C0182d.f422d, openConnection.getContentType());
                }
                if (headerCallback) {
                    errorCallback(this.f2230b, this.f2231c + " aborted");
                    return;
                }
                int contentLength = openConnection.getContentLength() > 0 ? openConnection.getContentLength() : 0;
                i = (url.getProtocol().equalsIgnoreCase("file") || url.getProtocol().equalsIgnoreCase("jar")) ? contentLength == 0 ? WXMediaMessage.THUMB_LENGTH_LIMIT : Math.min(contentLength, WXMediaMessage.THUMB_LENGTH_LIMIT) : 1428;
                int i3 = 0;
                try {
                    String str;
                    ...
                    if (obj != null) {
                        errorCallback(this.f2230b, str);
                    }
                    progressCallback(0, 0, i3, i3, 0, 0);
                    doneCallback(this.f2230b);
                    return;
                } catch (Exception e3) {
                    errorCallback(this.f2230b, e3.toString());
                    return;
                }
            }
            errorCallback(this.f2230b, url.getHost() + url.getFile() + " is not an absolute path!");
        } catch (MalformedURLException e4) {
            errorCallback(this.f2230b, e4.toString());
        } catch (IOException e22) {
            errorCallback(this.f2230b, e22.toString());
        }
    }
}
