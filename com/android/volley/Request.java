    ...
    void finish(String tag) {
        ...
        if (MarkerLog.ENABLED) {
            ...
            if (Looper.myLooper() != Looper.getMainLooper()) {
                new Handler(Looper.getMainLooper()).post(new C02171(tag, threadId));
                return;
            }
        ...
        if (SystemClock.elapsedRealtime() - this.mRequestBirthTime >= SLOW_REQUEST_THRESHOLD_MS) {
            VolleyLog.m716d("%d ms: %s", Long.valueOf(SystemClock.elapsedRealtime() - this.mRequestBirthTime), toString());
        }
    }
