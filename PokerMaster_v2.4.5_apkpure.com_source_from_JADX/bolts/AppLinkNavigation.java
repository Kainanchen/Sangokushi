...
public class AppLinkNavigation {
    ...
    static class C02391 implements Continuation<AppLink, NavigationResult> {
        final /* synthetic */ Context val$context;

        static {
            fixHelper.fixfunc(new int[]{183, 184, 185});
            if (VERSION.SDK_INT <= 0) {
                Exists2.class.toString();
            }
        }

        native C02391(Context context);

        public final native NavigationResult then(Task<AppLink> task);

        public final native /* bridge */ /* synthetic */ Object m5956then(Task task);
    }
    ...
    private void sendAppLinkNavigateEventBroadcast(Context context, Intent intent, NavigationResult navigationResult, JSONException jSONException) {
        ...
        hashMap.put(MessageKey.MSG_TYPE, navigationResult.getCode());
    ...
    public static Task<NavigationResult> navigateInBackground(Context context, Uri uri, AppLinkResolver appLinkResolver) {
        return appLinkResolver.getAppLinkFromUrlInBackground(uri).onSuccess(new C02391(context), Task.UI_THREAD_EXECUTOR);
    }
