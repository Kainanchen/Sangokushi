package com.amap.p003a;

import com.amap.p003a.DexFileManager.DexFileManager;
import java.io.ByteArrayInputStream;
import java.io.Closeable;
import java.security.PublicKey;
import java.security.cert.CertificateFactory;
import java.security.cert.X509Certificate;
import java.util.List;

/* compiled from: Utils */
/* renamed from: com.amap.a.ac */
public final class ac {
    public static int m103a(String str, String str2) {
        int i = 0;
        try {
            String[] split = str.split("\\.");
            String[] split2 = str2.split("\\.");
            int min = Math.min(split.length, split2.length);
            for (int i2 = 0; i2 < min; i2++) {
                i = split[i2].length() - split2[i2].length();
                if (i != 0) {
                    break;
                }
                i = split[i2].compareTo(split2[i2]);
                if (i != 0) {
                    break;
                }
            }
            return i != 0 ? i : split.length - split2.length;
        } catch (Throwable e) {
            cm.m637a(e, "Utils", "compareVersion");
            return -1;
        }
    }

    static PublicKey m104a() {
        Closeable byteArrayInputStream;
        Throwable th;
        try {
            CertificateFactory instance = CertificateFactory.getInstance("X.509");
            byteArrayInputStream = new ByteArrayInputStream(cf.m581b(""));
            try {
                PublicKey publicKey = ((X509Certificate) instance.generateCertificate(byteArrayInputStream)).getPublicKey();
                ac.m105a(byteArrayInputStream);
                return publicKey;
            } catch (Throwable th2) {
                th = th2;
                try {
                    cm.m637a(th, "DyLoader", "init");
                    ac.m105a(byteArrayInputStream);
                    return null;
                } catch (Throwable th3) {
                    th = th3;
                    ac.m105a(byteArrayInputStream);
                    throw th;
                }
            }
        } catch (Throwable th4) {
            th = th4;
            byteArrayInputStream = null;
            ac.m105a(byteArrayInputStream);
            throw th;
        }
    }

    static void m105a(Closeable closeable) {
        if (closeable != null) {
            try {
                closeable.close();
            } catch (Throwable th) {
                th.printStackTrace();
            }
        }
    }

    static void m106a(List<aa> list) {
        for (int i = 0; i < list.size() - 1; i++) {
            for (int i2 = i + 1; i2 < list.size(); i2++) {
                aa aaVar = (aa) list.get(i);
                aa aaVar2 = (aa) list.get(i2);
                if (ac.m103a(aaVar2.f78d, aaVar.f78d) > 0) {
                    list.set(i, aaVar2);
                    list.set(i2, aaVar);
                }
            }
        }
    }

    static boolean m107a(DBOperation dBOperation, String str, String str2, cj cjVar) {
        aa a = DexFileManager.m731a(dBOperation, str);
        return a != null && cjVar.m612b().equals(a.f77c) && ac.m108b(str2, a.f76b);
    }

    static boolean m108b(String str, String str2) {
        String a = cg.m584a(str);
        return a != null && a.equalsIgnoreCase(str2);
    }
}
